...
return new Bluebird((resolve, reject) => {
    idMap = new Map()
    let fixtureFile = fixtureName

    return sequelizeFixtures.loadFile(fixtureFile, models, {
    log: fixtureLog,
    modifyFixtureDataFn: function (data, Model) {
        if (!Model.hasHook('beforeCreate')) {
        Model.beforeCreate((record) => {
            logger.debug('(beforeCreate) Model.name=' + Model.name + ' id=' + record.dataValues.id + ' desc=' + record.dataValues.description)
            // a key that needs to be stored?
            if (record.dataValues.id && isNaN(record.dataValues.id) && record.dataValues.id.startsWith(Model.name)) {
            logger.debug('Replacement id, ' + record.dataValues.id + ' found')

            idMap.set(record.dataValues.id, null)
            Object.defineProperty(record, 'replacementId', {
                value: record.dataValues.id,
                enumerable: false,
                writable: false
            })
            record.dataValues.id = null
            }
        })
        }

        if (!Model.hasHook('afterCreate')) {
        Model.afterCreate((record) => {
            if (record.replacementId) {

            idMap.set(record.replacementId, record.dataValues.id)
            logger.debug('(afterCreate) ' + record.replacementId + ' was saved as id ' + record.dataValues.id)
            }
        })
        }
        ...
        if (key.endsWith('Id')) {
            logger.debug('(foreach) looking for id to replace key:' + data[key])
            let replacementId = data[key]

            data[key] = idMap.get(data[key])

            logger.debug('Replaced ' + replacementId + ' with ' + data[key])
        }
        })
    ...
    logger.debug('Fixture loaded')
    for (const entry of idMap.entries()) {
        logger.debug(entry)
    }

    delete require.cache[path.resolve(process.cwd(), fixtureFile)]
